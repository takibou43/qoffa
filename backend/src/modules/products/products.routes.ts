...
import { barcodeLookupLimiter, writeLimiter } from '../../middleware/rateLimit.js';
...
/** مسح/إدخال باركود: قُفّة أولًا ثم Open Food Facts ثم UPCitemdb — NEW | AVAILABLE_TO_ADD | ALREADY_LISTED */
productsRouter.get('/barcode/:code', barcodeLookupLimiter, async (req, res) => {
  const shop = await getOwnedShopOrThrow(req.auth!.userId);
  ...
  // المصادر الخارجية (وإنشاء منتج عالمي منها) للمحلات المعتمدة فقط
  res.json(
    await service.lookupBarcode(shop.id, parsed.data.code, {
      allowExternal: shop.status === 'APPROVED',
    }),
  );
});
